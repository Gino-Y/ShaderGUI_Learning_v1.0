...
- User audit found generated subtitles/audio leaking transcript outline labels such as `核心观点`, `问题：`, and `思路：`.
- Root cause: transcript Markdown used editorial section labels and `generate_audio.py clean_text()` only stripped markdown markers, leaving label text to be spoken.
- Workflow fix: `.agent/templates/scripts/generate_audio.py` and `scripts/generate_audio.py` now remove metadata headings, outline headings, table rows, list labels, and internal production fields before subtitle/TTS generation.
- Verification fix: `scripts/verify_course.py` and `.agent/templates/scripts/verify_course.py` now check the cleaned narration surface for forbidden outline labels.
- Product artifacts must be regenerated by MVP/ADP; generated subtitles/audio were not hand-edited.
## 2026-05-06 GitHub Pages Deployment Config

- Deployment target: GitHub Pages project site under `/ShaderGUI_Learning_v1.0/`.
